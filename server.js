...
async function checkRole(req, res, next) {
  try {
    const userResult = await pool.query(
      'SELECT user_type, dispatch_center_id FROM users WHERE id = $1',
      [req.userId]
    );
...
app.post('/api/auth/verify-code', authLimiter, async (req, res) => {
  try {
    ...
    if (codeResult.rows.length === 0) return res.status(400).json({ error: 'No verification code found' });
    ...
    res.json({
      success: true, token,
      user: { id: user.id, phone: user.phone, user_type: user.user_type, dispatch_center_id: user.dispatch_center_id }
    });
  } catch (err) {
    console.error(err);
...
app.patch('/api/emergencies/:id/confirm', authenticateToken, checkRole, async (req, res) => {
  try {
    if (req.userType !== 'dispatcher') return res.status(403).json({ error: 'Only dispatchers can confirm emergencies' });
    const emergencyResult = await pool.query('SELECT dispatch_center_id FROM emergencies WHERE id = $1', [req.params.id]);
    if (emergencyResult.rows.length === 0) return res.status(404).json({ error: 'Emergency not found' });
...
// Caller cancels — works on ANY active status
app.patch('/api/emergencies/:id/cancel', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `UPDATE emergencies 
       SET status = 'cancelled', cancelled_by = 'user', rejected_at = NOW() 
       WHERE id = $1 AND user_id = $2 
       AND status NOT IN ('completed', 'cancelled', 'rejected')
       RETURNING *`,
      [req.params.id, req.userId]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Not found or cannot cancel' });
    res.json(result.rows[0]);
  } catch (err) {
...
app.patch('/api/emergencies/:id/assign-ambulance', authenticateToken, checkRole, async (req, res) => {
  try {
    ...
    if (req.userType !== 'dispatcher') return res.status(403).json({ error: 'Only dispatchers can assign ambulances' });
...
app.get('/api/ambulances', authenticateToken, checkRole, async (req, res) => {
  try {
    if (req.userType !== 'dispatcher') return res.status(403).json({ error: 'Only dispatchers can view ambulances' });
    const result = await pool.query(
...
// ✅ NEW: Public ambulance location — for caller app to track driver
...
app.get('/api/driver/assigned-call', authenticateToken, async (req, res) => {
  try {
    const ambulanceResult = await pool.query('SELECT id FROM ambulances WHERE driver_user_id = $1', [req.userId]);
    if (ambulanceResult.rows.length === 0) return res.json({ call: null });
    const ambulanceId = ambulanceResult.rows[0].id;
...
app.patch('/api/driver/location', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) return res.status(404).json({ error: 'No ambulance linked to this driver' });
    res.json({ success: true });
  } catch (err) {
...
app.patch('/api/driver/arrived/:callId', authenticateToken, async (req, res) => {
  try {
    ...
    const emergencyResult = await pool.query(
      'SELECT id FROM emergencies WHERE id = $1 AND assigned_ambulance_id = $2', [callId, ambulanceId]
    );
    if (emergencyResult.rows.length === 0) return res.status(404).json({ error: 'Call not found' });
    await pool.query("UPDATE emergencies SET status = 'arrived', arrived_at = NOW() WHERE id = $1", [callId]);
    res.json({ success: true });
  } catch (err) {
...
app.patch('/api/driver/complete/:callId', authenticateToken, async (req, res) => {
  try {
    ...
    if (emergencyResult.rows.length === 0) return res.status(404).json({ error: 'Call not found' });
    await pool.query("UPDATE emergencies SET status = 'completed', completed_at = NOW() WHERE id = $1", [callId]);
    res.json({ success: true });
  } catch (err) {
...
app.get('/api/driver/call-history', authenticateToken, async (req, res) => {
  try {
    const ambulanceResult = await pool.query('SELECT id FROM ambulances WHERE driver_user_id = $1', [req.userId]);
    if (ambulanceResult.rows.length === 0) return res.json({ calls: [] });
    const ambulanceId = ambulanceResult.rows[0].id;
...
app.post('/api/driver/accept-call/:callId', authenticateToken, async (req, res) => {
  try {
    ...
    const checkResult = await pool.query(
      "SELECT id FROM emergencies WHERE id = $1 AND status = 'confirmed'", [callId]
    );
    if (checkResult.rows.length === 0) return res.status(409).json({ error: 'Bu chaqiruv allaqachon qabul qilingan' });
    ...
    if (result.rows.length === 0) return res.status(409).json({ error: 'Bu chaqiruv allaqachon qabul qilingan' });
