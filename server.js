...
app.patch('/api/emergencies/:id/reject', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rows[0].assigned_ambulance_id) {
      await pool.query("UPDATE ambulances SET status = 'available' WHERE id = $1", [result.rows[0].assigned_ambulance_id]);
    }
    res.json(result.rows[0]);
  } catch (err) {
    console.error(err);
...
app.patch('/api/emergencies/:id/cancel', authenticateToken, async (req, res) => {
  try {
    ...
    // Reset ambulance status back to available
    if (result.rows[0].assigned_ambulance_id) {
      await pool.query("UPDATE ambulances SET status = 'available' WHERE id = $1", [result.rows[0].assigned_ambulance_id]);
    }
    res.json(result.rows[0]);
...
app.get('/delete-account', (req, res) => res.sendFile(path.join(__dirname, 'delete-account.html')));
...
app.patch('/api/driver/start/:callId', authenticateToken, async (req, res) => {
  try {
    ...
    await pool.query("UPDATE ambulances SET status = 'on_the_way' WHERE id = $1", [ambId]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
...
app.patch('/api/driver/arrived/:callId', authenticateToken, async (req, res) => {
  try {
    ...
    await pool.query("UPDATE ambulances SET status = 'arrived' WHERE id = $1", [ambId]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
